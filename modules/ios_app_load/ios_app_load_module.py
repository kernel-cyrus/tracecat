...
class Ios_app_load_module(Module):
    ...
    def get_desc(self):
        ...
        return 'Process cpu usage on each core, for iPhone. (Parse from trace)'
    ...
    * 需要在MacOS运行，需要安装xcode软件'''
    ...
    def do_parse(self, params):
        ...
        process_list = self.instruments.get_process_list()
        
        sched_slices = self.instruments.get_cpu_state().to_dict(orient='records')
        ...
        while window := pick_next_window(window, trace_info['start_time'], trace_info['end_time'], window_time, True):
            ...
            for proc in process_list:
                ...
                for cpu_id in trace_info['cpu_list']:
                    ...
                    records[proc['process_id']]['cpu_time_' + str(cpu_id)] = 0
                    records[proc['process_id']]['cpu_load_' + str(cpu_id)] = 0
            ...
            for row in slices:
                ...
                records[row['process_id']]['cpu_time_' + str(row['cpu_id'])] += row['duration']
    ...
    def __get_proc_list(self, df):
        ...
        proc_list = get_unique_list(df, {'proc_id': str, 'proc_name': str}, skip_none = True) # None means interrupt, skip it.

        return sorted(proc_list, key = lambda i: int(i['proc_id']))
    ...
    def __print_proc_list(self, proc_list):
        ...
        for proc in proc_list:
            ...
            print('\t' + proc['proc_id'] + '\t' + proc['proc_name'])
